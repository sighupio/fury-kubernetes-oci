resource "oci_core_instance" "infra" {
  ...
  display_name   = "infra-${count.index+1}"
  hostname_label = "infra-${count.index+1}"
...
resource "oci_core_instance" "staging" {
  ...
  display_name = "staging-${count.index+1}"

  // fault_domain = "${var.instance_fault_domain}"

  hostname_label = "staging-${count.index+1}"
  metadata {
    ssh_authorized_keys = "${file(var.ssh-public-key-path)}"
  }
  source_details {
...
resource "oci_core_instance" "production" {
  ...
  display_name = "production-${count.index+1}"

  hostname_label = "production-${count.index+1}"
